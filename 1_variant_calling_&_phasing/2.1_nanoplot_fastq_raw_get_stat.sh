...
# get QC stats from nanoplot results

# chmod 755 2_nanoplot_fastq_raw_get_stat.sh 
# ./2.1_nanoplot_fastq_raw_get_stat.sh


Sample=""
wkdir=""
dir=$wkdir/2_nanoplot_fastq_raw
...
echo -e $Sample"\t"raw"\t"$Mean_read_length"\t"$Mean_read_quality"\t"$Median_read_length"\t"$Median_read_quality"\t"$Number_of_reads"\t"$Read_length_N50"\t"$Stdev_read_length"\t"$Total_bases"\t"$Q5"\t"$Q7"\t"$Q10"\t"$Q12"\t"$Q15>>clean_stat.txt