...
$L1=length($myscript);
$L2=length($0);
$workingfolder=substr $myscript, 0, ($L1 - $L2 - 23);
...
print "Starting Segmentation and Calling...\n";
 system qq(R --slave --args $Main_Output_Folder_Path,$R_Target_Folder,$Input_File_Path,$Mode,$Target_Name,$Program_Folder_Path,$Assembly < $Program_Folder_Path/lib/R/NanoGLADIATORInferenceGenome.R);
print "Segmentation and Calling Complete\n";



#system qq(R --slave --args $Main_Output_Folder_Path,$Input_File_Path < $Program_Folder_Path/lib/R/XCAVATORPlots.R);
