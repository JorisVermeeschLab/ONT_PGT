...
if (mode!="paired")
{
  ...
  for (i in 1:length(ExpPart))
  {
    ...
    ShVector<-c(ShVector,paste("perl ",file.path(ProgramFolder,"lib","perl","RCAnalysis.pl "),FileOut,SettingLabel,sep=""))
    StartInd<-EndInd+1
  }
...
FileShOut<-file.path(PathOut,"ParallelRCAnalysis.sh")
