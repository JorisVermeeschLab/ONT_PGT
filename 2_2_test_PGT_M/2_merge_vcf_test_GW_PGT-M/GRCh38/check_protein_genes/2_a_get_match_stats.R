...
file<-paste(sample,".protein_genes.vcf_filtered_revised.txt",sep="")
data<-read.table(file,header = T)
info<-read.table("/lustre1/project/stg_00019/research/yan/nanopore_data/00_02_resource_GTF/GRCh38/gene_list_with_info.txt",header=T)
info$gene_len_bp<-info$end-info$start+1

data_gene <- left_join(data,info, join_by(CHROM==seqname,POS >= start,POS <= end)) %>%
                select(-start,-end) 

# write.table(test,file="gene.txt")

 # replace . in PS_** columns with value from the surrounding line
data_ps_imputed<-data_gene %>%
  group_by(CHROM) %>%
...
# merge CHROM & gene ID & PS  to a single column 
data_ps_imputed$PS_merged<-paste(data_ps_imputed$CHROM,data_ps_imputed$gene_id,data_ps_imputed$PS_HG003,data_ps_imputed$PS_HG004,data_ps_imputed$PS_HG002)
...
# 1. group by PS_merged (CHROM & gene ID & PS)
...
clean_trio_PS_stats<-trio_PS_hp_stats %>%
  select(CHROM,start,end,gene_name,gene_id,gene_len_bp,len_bp,len_snv,a_m_3_Ha,a_m_3_Hb,a_m_4_Ha,a_m_4_Hb,b_m_3_Ha,b_m_3_Hb,b_m_4_Ha,b_m_4_Hb,unique_hp_FP,unique_hp_3,unique_hp_4,rep_3_Ha,rep_3_Hb,rep_4_Ha,rep_4_Hb)
