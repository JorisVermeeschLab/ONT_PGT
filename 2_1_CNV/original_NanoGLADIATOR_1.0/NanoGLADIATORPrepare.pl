...
my $Input_File_Parallel="$Temp_Folder/ParallelBamPrepare.sh";
...
=head1 SYNOPSIS 
 ...
 NanoGLADIATORPrepare.pl [arguments] [options]
...
Function:
 ...
 perl NanoGLADIATORPrepare.pl performs RC calculations, data normalization and data analysis on multiple .bam files present in the FilePrepareExample.txt file.

 Example: 
 
 NanoGLADIATOR> perl NanoGLADIATORPrepare.pl FilePrepareExample.txt --mapq --processors 6 --target w1000 --assembly hg19 
...
The number of processors used to perform data preparation. Parallelism is performed on samples. 
...
NanoGLADIATORPrepare.pl is a Perl script which is part of the Nano-GLADIATOR package. It performs RC calculations for each consecutive and non-overlapping window of size L, and then perfom GC-content and mappability normalization on multiple .bam files.
