...
for (i in 1:length(ExpPart))
{
  ...
  ShVector<-c(ShVector,paste("perl ",file.path(ProgramFolder,"lib","perl","BamPrepare.pl "),FileOut,SettingLabel,sep=""))
  StartInd<-EndInd+1
}


FileShOut<-file.path(PathOut,"ParallelBamPrepare.sh")
