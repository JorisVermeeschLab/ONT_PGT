...
for idx,c in enumerate(chromosmes):
    ...
    trace5=go.Scatter(
        ...
        mode='lines',
        showlegend=False,
        text=map(lambda x:str(x), [round(i,2) for i in seg if i is not None]),
    ...
    c_counter += 1
    last_idx = len(zip(*CallsTab)[0])+last_idx
for sd in supp_data:
    fig.data.append(sd)
...
trace6=go.Scatter(
    x=map(int, zip(*seg_plot)[0]),
    y=[round(i,2) for i in map(float, zip(*seg_plot)[1]) if i is not None],
    name='Segmentation',
    mode='lines',
    showlegend=True,
    text=map(lambda x:str(x), [round(i,3) for i in map(float, zip(*seg_plot)[1]) if i is not None]),
    hoverinfo="text+name",
...
updatemenus=list([
        dict(
            buttons=list([
                dict(
                    args=[{'visible': ListChanger([item for sublist in FilteringIndeces for item in sublist])}, {'annotations': map(lambda x:plotAnnotesy1[x], [item-len(chromosmes) for sublist in FilteringIndeces for item in sublist] + [len(plotAnnotesy1)-1])}],
                    label='All',
                    method='update'
                ),
                dict(
                    args=[{'visible': ListChanger([item for sublist in FilteringIndeces[1:] for item in sublist])}, {'annotations': map(lambda x:plotAnnotesy1[x], [item-len(chromosmes) for sublist in FilteringIndeces[1:] for item in sublist] + 
[len(plotAnnotesy1)-1])}],
                    label='10%',
                    method='update'
                ),
                dict(
                    args=[{'visible': ListChanger([item for sublist in FilteringIndeces[2:] for item in sublist])}, {'annotations': map(lambda x:plotAnnotesy1[x], [item-len(chromosmes) for sublist in FilteringIndeces[2:] for item in sublist] + 
[len(plotAnnotesy1)-1])}],
                    label='20%',
                    method='update'
                ),
                dict(
                    args=[{'visible': ListChanger([item for sublist in FilteringIndeces[3:] for item in sublist])}, {'annotations': map(lambda x:plotAnnotesy1[x], [item-len(chromosmes) for sublist in FilteringIndeces[3:] for item in sublist] + 
[len(plotAnnotesy1)-1])}],
                    label='30%',
                    method='update'
                ),
                dict(
                    args=[{'visible': ListChanger([item for sublist in FilteringIndeces[4:] for item in sublist])}, {'annotations': map(lambda x:plotAnnotesy1[x], [item-len(chromosmes) for sublist in FilteringIndeces[4:] for item in sublist] + 
[len(plotAnnotesy1)-1])}],
                    label='40%',
                    method='update'
                ),
                dict(
                    args=[{'visible': ListChanger([item for sublist in FilteringIndeces[5:] for item in sublist])}, {'annotations': map(lambda x:plotAnnotesy1[x], [item-len(chromosmes) for sublist in FilteringIndeces[5:] for item in sublist] + 
[len(plotAnnotesy1)-1])}],
                    label='50%',
                    method='update'
                ),
                dict(
                    args=[{'visible': ListChanger([item for sublist in FilteringIndeces[6:] for item in sublist])}, {'annotations': map(lambda x:plotAnnotesy1[x], [item-len(chromosmes) for sublist in FilteringIndeces[6:] for item in sublist] + 
[len(plotAnnotesy1)-1])}],
                    label='60%',
                    method='update'
                ),  
                dict(
                    args=[{'visible': ListChanger([item for sublist in FilteringIndeces[7:] for item in sublist])}, {'annotations': map(lambda x:plotAnnotesy1[x], [item-len(chromosmes) for sublist in FilteringIndeces[7:] for item in sublist] + 
[len(plotAnnotesy1)-1])}],
                    label='70%',
                    method='update'
                ),
                dict(
                    args=[{'visible': ListChanger([item for sublist in FilteringIndeces[8:] for item in sublist])}, {'annotations': map(lambda x:plotAnnotesy1[x], [item-len(chromosmes) for sublist in FilteringIndeces[8:] for item in sublist] + 
[len(plotAnnotesy1)-1])}],
                    label='80%',
                    method='update'
                ),
                dict(
                    args=[{'visible': ListChanger([item for sublist in FilteringIndeces[9:] for item in sublist])}, {'annotations': map(lambda x:plotAnnotesy1[x], [item-len(chromosmes) for sublist in FilteringIndeces[9:] for item in sublist] + 
[len(plotAnnotesy1)-1])}],
                    label='90%',
                    method='update'
                ), 
                dict(
                    args=[{'visible': ListChanger([item for sublist in [] for item in sublist])}, {'annotations': map(lambda x:plotAnnotesy1[x], [item-len(chromosmes) for sublist in [] for item in sublist] + [len(plotAnnotesy1)-1])}],
                    label='None',
                    method='update'
                ),                      
            ]),
